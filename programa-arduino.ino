...
// Configuraciones
const int servo1cerrado = 96;
const int servo1abierto = 0;
const int servo2cerrado = 73;
const int servo2abierto = 170;



float distanciaEntrada = 5.4;        // Distancia (cm) mínima para detectar un coche en entrada
float distanciaSalida =3.10;       // Distancia (cm) mínima para detectar un coche en salida
...
bool mantenerBarrera(int trig, int echo) {
  ...
  // Usar distanciaEntrada para el sensor de entrada (trig1/echo1)
  // y distanciaSalida para el sensor de salida (trig2/echo2)
  if (trig == trig1) {
    return (distancia <= distanciaEntrada) || (distancia > 7); // Detectar si está dentro del rango de entrada o fuera de rango (>50cm)
  } else {
    return (distancia <= distanciaSalida) || (distancia > 7); // Detectar si está dentro del rango de salida o fuera de rango (>50cm)
  }
}

// Detectar coche usando sensor ultrasónico
bool detectarCoche(int trig, int echo) {
    const int LECTURAS = 5;  // Número de lecturas para confirmar
    int lecturas = 0;        // Contador de lecturas positivas
    lecturas = 0;
    for(int i = 0; i < LECTURAS; i++) {
        float distancia = medirDistancia(trig, echo);
        if (trig == trig1) {
            if((distancia <= distanciaEntrada)) {
                lecturas++;
            }
        } else {
    ...
    return lecturas >= 3;  // Retorna true si la mayoría de lecturas detectan coche
}
...
float medirDistancia(int trig, int echo) {
  ...
  return distancia > 200 ? 200 : distancia;
}
...
void logs() {
  if (millis() - ultimoLog >= 1000) {
    ...
    float distanciaMedidaEntrada = medirDistancia(trig1, echo1);
    float distanciaMedidaSalida = medirDistancia(trig2, echo2);
    
    // Resumen de plazas
    int plazasLibres = 0;
    for (int i = 0; i < 6; i++) if(plazas[i]) plazasLibres++;
    ...
    Serial.print(distanciaMedidaEntrada);
    Serial.println(" cm");
    
    Serial.print("Sensor Salida: ");
    Serial.print(distanciaMedidaSalida);
    Serial.println(" cm");
...
void gestionarPantalla() {
    ...
    switch (random(0,2)) {
        ...
        case 1: {
            lcd.print("Precio/min: ");
            lcd.print(precioMinuto, 2);
            lcd.print(char(0xE2));
            lcd.setCursor(0, 1);
            lcd.print("Plazas: ");
            lcd.print(libres);
